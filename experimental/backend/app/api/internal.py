...
# ---------------------------------------------------------------------------
# MVP — Feishu Bot control surface (simplify §10 / §12 / §13 / §14)
#
# Three endpoints back the 5 MVP Feishu bot commands. Each is mounted
# under `/api/internal` (X-Radar-Webhook protected) so they can also be
# triggered from curl / n8n / the operator manual:
#
#   /pipeline/run         ← /run    (manual kickoff)
#   /status               ← /status (read latest Run row + source health)
#   /sources/healthy      ← /sources (per-source health snapshot)
#
# These endpoints are intentionally minimal — they compose the existing
# IngestionService + RunRepository rather than building a parallel
# pipeline. The Run table is the single source of truth for the
# `/status` reply.
# ---------------------------------------------------------------------------
...
class _PipelineRunRequest(BaseModel):
    ...
    All fields default to the "run the full enabled set" path so a bare
    POST from the bot still does something useful.
    """
...
@router.post(
    "/pipeline/run",
    summary="MVP /run — kick off a full collection+AI+Feishu pipeline",
)
async def pipeline_run(
    ...
    """Synchronous MVP version of the daily cron pipeline.

    The full Phase 7 pipeline is asynchronous (research + content
    generation + multi-channel publish) — MVP keeps it sequential and
    synchronous so the Feishu `/run` reply is honest about completion.
    """
    ...
    try:
        ingestion = IngestionService(
            session,
            source_slugs=body.source_slugs,
        )
        report = await ingestion.run_once()
        raw_count = int(report.items_seen)
        new_count = int(report.items_inserted)
        ...
        if body.send_digest:
            ...
            digest_sent = bool(await notifier.send_daily_digest())
        ...
        try:
            await repo.finish_failed(run, error=error)
            await session.commit()
...
async def pipeline_status(
    ...
    """Read the latest ``Run`` row + a snapshot of source health.

    The Feishu `/status` command renders this verbatim into a
    human-readable Chinese summary (see ``services/feishu/inbound.py``).
    """
    from sqlalchemy import func
    ...
    last_run = await run_repo.latest()
    ...
    healthy = sum(1 for s in source_rows if s.last_success_at is not None)
    total_sources = len(source_rows)

    total_signals = int(
        (await session.execute(select(func.count(Signal.id)))).scalar_one() or 0
    )
        ...
        "last_run": (
            {
                ...
                "raw_count": last_run.raw_count,
                "new_count": last_run.new_count,
                "signal_count": last_run.signal_count,
            ...
            "items": [
                {
                    ...
                    "last_error_at": (
                        s.last_error_at.isoformat() if s.last_error_at else None
                    ),
...
@router.get(
    "/sources/healthy",
    summary="MVP /sources — per-source health snapshot for the bot reply",
)
async def sources_healthy(
    session: AsyncSession = Depends(get_session),
    _secret: None = Depends(require_admin),
) -> dict[str, Any]:
    """List every enabled source with its last-success / last-error
    timestamps. The Feishu `/sources` command renders a simple
    ``✓ name | last success at`` table from this payload.
    """
    ...
    items = [
        {
            ...
            "type": s.type,
            "url": s.url,
            "healthy": s.last_success_at is not None,
            "last_success_at": (
                s.last_success_at.isoformat() if s.last_success_at else None
            ),
            "last_error_at": (
    ...
    return {
        "count": len(items),
        "healthy": sum(1 for it in items if it["healthy"]),
        "items": items,
    }
